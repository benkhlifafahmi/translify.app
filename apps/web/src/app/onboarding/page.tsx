      ...
      <div className="stagger">
        ...
        <div className="mt-8 flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-[0.72rem] font-semibold uppercase tracking-[0.12em] text-[color:var(--color-ink-soft)]">
              {t("ob.s5.or")}
            </span>
            <div className="h-px flex-1" style={{ background: "var(--color-border)" }} />
          </div>
...
// One-click Google sign-up. Routes through the existing Google OAuth callback,
// which lands the new user in /library?welcome=1 (trial path). Users who
// specifically want to subscribe to a plan continue with email + password
// below and click "Subscribe": Stripe Checkout captures payment after
// register() succeeds, which the OAuth-redirect cannot currently do.
function GoogleSignup() {
  ...
  const handleClick = async () => {
    setLoading(true);
    posthog?.capture("onboarding_google_clicked");
    try {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="flex h-14 w-full items-center justify-center gap-3 rounded-2xl border-2 font-[family-name:var(--font-display)] text-[1rem] font-semibold transition-[transform,box-shadow] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-translate-y-[1px] active:scale-[0.98] active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50"
      style={{
        borderColor: "var(--color-border-strong)",
        background: "white",
        color: "var(--color-ink)",
        boxShadow: loading ? "none" : "0 3px 0 rgba(74,60,30,0.10)",
      }}
    >
      {loading ? (
      ...
      {loading ? t("ob.s5.googleRedirecting") : t("ob.s5.googleCta")}
    </button>
  );
}
