...
export default function GoogleCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (error || !code || !state) {
      setErrMsg(error === "access_denied" ? "You cancelled the Google sign-in." : "Google login failed. Please try again.");
      setStatus("error");
      return;
    }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5" style={{ color: "var(--color-ink)" }}>
        ...
        <span className="font-[family-name:var(--font-display)] text-[1.05rem] font-semibold tracking-tight">Translify</span>
      </div>

      {status === "loading" ? (
        <div className="flex flex-col items-center gap-4">
          <Lumi state="thinking" size={96} animate />
          <p className="font-[family-name:var(--font-display)] text-[1rem] font-medium" style={{ color: "var(--color-ink-soft)" }}>
        ...
        <div
          ...
          style={{ borderColor: "var(--color-border)", background: "white", boxShadow: "0 6px 0 rgba(74,60,30,0.10)" }}
        >
          <Lumi state="sad" size={80} animate />
          <h2 className="mt-4 font-[family-name:var(--font-display)] text-[1.3rem] font-semibold" style={{ color: "var(--color-ink)" }}>
            Oops, something went wrong
          </h2>
          <p className="mt-2 text-[0.9rem]" style={{ color: "var(--color-ink-soft)" }}>{errMsg}</p>
          <button
            type="button"
            onClick={() => router.push("/join")}
            className="mt-6 h-11 w-full rounded-2xl font-[family-name:var(--font-display)] text-[0.95rem] font-bold text-white"
            style={{ background: "linear-gradient(to bottom,#EDB86A,#D09040)", boxShadow: "0 5px 0 rgba(152,96,24,0.45)" }}
          >
            Try again →
          </button>
        </div>
