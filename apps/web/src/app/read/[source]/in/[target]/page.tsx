// /read/{source}/in/{target} — 56 programmatic SEO pages, one per language pair.
//
// Note on URL shape: Next.js 15 requires each dynamic [param] to be its own
// folder segment, so the URL is /read/spanish/in/english rather than the
// flatter /read-spanish-books-in-english. SEO impact is comparable — the
// keyword tokens "read", source, "in", target are all still in the path.
...
interface Params { source: LangSlug; target: LangSlug }
...
export async function generateMetadata({
  ...
  if (!(source in LANGS) || !(target in LANGS) || source === target) return {};
...
export default async function ReadSourceInTargetPage({
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${src.label} → ${tgt.label}`, item: `${SITE}/read/${source}/in/${target}` },
    ],
  };
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <nav className="mb-6 text-sm text-[color:var(--color-ink-soft)]">
        ...
        {" · "}
        <Link href="/languages" className="hover:text-[color:var(--color-ink)]">Languages</Link>
        {" · "}
      ...
      <section className="mt-14">
        <h2 className="font-[family-name:var(--font-display)] text-2xl font-semibold">
          Other {src.label} reading pairs
        </h2>
        <p className="mt-2 text-[color:var(--color-ink-soft)]">
          Translify supports 56 pairs across 8 languages.
          {" "}<Link href="/languages" className="font-semibold underline decoration-[color:var(--color-saffron)]">Browse them all →</Link>
        </p>
