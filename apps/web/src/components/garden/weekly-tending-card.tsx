...
export function WeeklyTendingCard({ garden }: { garden: Garden }) {
  ...
  const dueText = dueMs <= 0
    ...
    : dueMs < 24 * 3_600_000
      ? "due in under a day"
      : `due in ${Math.ceil(dueMs / (24 * 3_600_000))} days`;
  ...
  return (
    <div className="relative overflow-hidden rounded-sm border border-[color:var(--color-ink)] bg-[linear-gradient(180deg,#FBF3DD_0%,#F4E9C9_100%)] p-6">
      ...
      <p className="mb-4 max-w-[88%] text-sm italic text-[color:var(--color-ink-soft)]">
        Five questions on chapters {garden.pagesRead > 200 ? "9 — 14" : "1 — 8"}. Miss the window
        and {speciesShort(garden)} will begin to wilt.
      </p>
...
function speciesShort(garden: Garden): string {
  switch (garden.species) {
    case "ficus": return "Ficus litteraria";
    case "helianthus": return "your sunflower";
