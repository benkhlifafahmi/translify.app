...
export function MindmapTool({ bookId }: { bookId: string }) {
  ...
  async function generate() {
    ...
    setErr(null);
    try {
      const root = await generateMindMap(bookId, trimmed, null);
      const map = makeMap(trimmed, root);
      store.saveMap(map);
      setActive(map);
      setTopic("");
    } catch {
      setErr(t("study.map.genError"));
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="flex flex-col gap-5 p-5">
      ...
      <div className="flex flex-col gap-2 rounded-2xl border-[1.5px] border-[color:var(--color-border)] bg-[#FFFCF3] p-4">
        ...
        <input
          ...
          placeholder={t("study.map.topicPlaceholder")}
          disabled={busy}
            ...
            {store.maps.map((m) => (
              <li key={m.id} className="group flex items-center gap-2 py-2.5">
                <button type="button" onClick={() => setActive(m)} className="min-w-0 flex-1 text-left">
                  ...
                  <p className="text-[0.72rem] text-[color:var(--color-ink-soft)]">{t("study.map.ideas", { n: countNodes(m.root) })}</p>
                </button>
                <button
                  type="button"
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-2 border-b border-[color:var(--color-border)] bg-[color:var(--color-paper)]/70 px-4 py-2.5">
        ...
        <span className="text-[0.7rem] font-semibold text-[color:var(--color-sage-deep)]">{t("study.map.savedBadge")}</span>
      </div>
      ...
      <div className="group flex items-center gap-1 rounded-lg px-1 py-1 transition-colors hover:bg-[color:var(--color-paper-3)]/40">
        {hasKids ? (
          <button
            ...
            title={node.collapsed ? t("study.map.expand") : t("study.map.collapse")}
        ...
        <span className="ml-1 hidden shrink-0 items-center gap-2 group-hover:inline-flex">
          <button type="button" onClick={() => onAdd(node.id)} title={t("study.common.add")} className="text-[color:var(--color-sage-deep)]">
            ＋
          </button>
          <button
            type="button"
